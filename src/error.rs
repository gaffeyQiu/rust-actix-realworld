    ...
    fn from(err: BcryptError) -> Self {
        match err {
            BcryptError::InvalidHash(_) => {
                AppError::Unauthorized(json!({"error": "Password is invalid"}))
            }
            _ => AppError::InternalServerError,
        }
    }
    ...
    fn from(err: JwtError) -> Self {
        match err.kind() {
            JwtErrorKind::InvalidToken => {
                AppError::Unauthorized(json!({"error": "Token is invalid"}))
            }
            JwtErrorKind::InvalidIssuer => {
                AppError::Unauthorized(json!({"error": "Issuer is invalid"}))
            }
            _ => AppError::InternalServerError,
        }
    }
